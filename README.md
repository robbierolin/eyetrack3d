...
3D gaze estimation is an open area of research.  The primary use-case of 3d gaze estimation would be probably be to extend the utility of eye trackers for usability studies.  This is already the most common use of eye tracking (maybe) and it stands to reason that 3d gaze estimation could extend the usefulness of eye-tracking based usability studies beyond web pages and screens. Secondly, 3d gaze estimation would allow for more types of eye-based interaction and richer user experience within VR and AR. Thirdly, foveated rendering could be extended to the more realistic setting of only rendering the region in space that falls on the fovea instead of the current strategy of rendering everything within some conical region that contains the 3d gaze position.
...
## Preprocessing
* Since left and right eye images and the cube positions were captured asynchronously, timestamps were used to create data points of the form (I_l, I_r, z) where I_l, and I_r are left and right eye images, respectively, and z is the z-position of the cube at close points in time.
...
* Output data was normalized to [0,1]
...
## Model
We learn a two-stream convolutional neural network. Each stream takes an image of either the left eye or right eye as input. The latent features of the images are merged before they are input to a fully connected layer that outputs the z coordinate of the cube position. All activation functions are RELU except the output layer has a sigmoid.
...
These are really good results.  Knowledge of eye behaviours (i.e. saccades and pursuits) could easily be incorporated time-series models built on top of these predictions to create robust, accurate trackers. Deep sequence models (e.g. LSTM) could also be used to learn this behaviour. Higher frequency capture devices would likely be needed. 